...
# MODIFIED: Using target value function
...
class ReplayBuffer():
    ...
    def put_episode( self, episode_data):
        for obs, act, logp, ret in episode_data:
            self.put((obs, act, logp, ret))

    def sample(self, n):
        mini_batch = random.sample(self.buffer, n)
        obs_lst, act_lst, logp_lst, ret_lst = [], [], [], []

        for transition in mini_batch:
            obs, act, logp, ret = transition
            obs_lst.append(obs)
            act_lst.append(act)
...
# TODO: Different learnign rates for each component
...
def sample():
    ...
    ep_obs, ep_acts, ep_logps, ep_rews = [], [], [], []
    ...
    for step in range( args.episode_length):
        ...
        if done:
            ...
            buffer.put_episode( zip( ep_obs, ep_acts, ep_logps, ep_returns))
...
while global_step < args.total_timesteps:
    ...
    obs_batch, act_batch, old_logp_batch, return_batch = buffer.sample( buffer.size())
    ...
    for i_epoch_pi in range( args.update_epochs):
        ...
        adv = return_batch - v_obs_batch
        clip_adv = torch.where( adv > 0,
                                (1.+args.clip_coef) * adv,
                                (1.-args.clip_coef) * adv)
        # DOUBT: There is no gradient for the clipped adv.
